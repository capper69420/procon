...
# Only these ~30 landmarks feed ROI hulls — smoothing all 468 was wasteful.
ROI_LANDMARK_INDICES: Tuple[int, ...] = tuple(sorted({
    *FOREHEAD_LANDMARKS, *LEFT_CHEEK_LANDMARKS, *RIGHT_CHEEK_LANDMARKS,
}))

# Reused erosion kernel (avoids per-frame np.ones allocation)
...
class LandmarkKalman:
    ...
    Per-frame cost drops from O(468) to O(|ROI_LANDMARK_INDICES|) Kalman updates.
...
class RppgProcessor:
   ...
   10. Heart rate from dominant FFT peak (bpm)
    ...
    def __init__(self, fps: float = RPPG_ASSUMED_FPS) -> None:
        ...
        # Pre-allocated ring buffers — avoids deque→array copy every process()
        ...
        # Cached Butterworth coefficients keyed by (fps, low, high)
        ...
        # Lazy-trained on first process() — avoids blocking startup ~2–5 s
    ...
    def _buffer_view(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        """Return chronologically ordered views without copying when full."""
        n = self._buf_count
        if n == 0:
        ...
        # Ring is full: concatenate tail + head for time-ordered slice
        h = self._buf_head
        return (
    ...
    def _get_bandpass_coefs(self, fps: float) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        """Cache butter() output — recomputing every 15 frames was redundant."""
        nyq = fps / 2.0
    ...
    def process(self, fps: Optional[float] = None) -> bool:
        ...
        # ── Step 6: bandpass filter (coefficients cached per fps) ─────────
        ...
        # ── Step 9: ML prediction (model trained lazily on first call) ───
    ...
    def _extract_features(
        ...
        # AC/DC ratio — compute each channel once (was 6 passes over R/G/B)
...
class FaceRoiResult:
    ...
    masks   : local sub-region masks (bh × bw), NOT full-frame — see rects
    rects   : absolute (x1, y1, x2, y2) crop bounds for each mask above
    ...
    def mean_rgb(self, frame: np.ndarray) -> Optional[Tuple[float, float, float]]:
        ...
        Replaces the old pattern of scanning full-frame boolean masks.
        """
        r_vals, g_vals, b_vals = [], [], []
    ...
    def extract(self, frame: np.ndarray) -> List[FaceRoiResult]:
        ...
        for det_idx, face_id in enumerate(face_ids):
            ...
            for group in self.roi_groups:
                ...
                # Sub-region mask: O(bw×bh) alloc instead of O(h×w) per ROI
                ...
                # Clip rect to frame boundaries to avoid shape mismatch with patch
                x1 = max(0, bx)
                y1 = max(0, by)
                x2 = min(w, bx + bw)
                y2 = min(h, by + bh)
                
                # Adjust mask to match clipped rect dimensions
                if x1 > bx or y1 > by or x2 < bx + bw or y2 < by + bh:
                    mask_h = y2 - y1
                    mask_w = x2 - x1
                    clipped_mask = local_mask[y1 - by:y1 - by + mask_h, x1 - bx:x1 - bx + mask_w]
                    local_mask = clipped_mask
...
class PhysNetLite(object):
    ...
    def predict(self, processor: "RppgProcessor") -> float:
        """Run inference on recent RGB ring buffer; returns SpO₂ estimate."""
        if not self.available or processor._buf_count < RPPG_MIN_FRAMES:
            return 0.0
        R, G, B = processor._buffer_view()
        seq = np.stack([R, G, B], axis=0)  # (3, T)
...
class PersonState:
    ...
    # O(1) vote tallies — avoids deque.count() + Counter each frame
    fallen_votes:   int = 0
...
def smooth_posture(state: PersonState, raw: str) -> str:
    ...
    Replaces deque.count() O(k) + list comprehension + Counter O(k) per frame.
    """
...
def draw_global_hud(frame, states: Dict[int, PersonState], fps: float):
    h, w = frame.shape[:2]
    n_total = len(states)
    n_standing = n_sitting = n_fallen = n_alert = 0
    for s in states.values():
        if s.posture == P.STANDING:
            n_standing += 1
        elif s.posture == P.SITTING:
            n_sitting += 1
        elif s.posture == P.FALLEN:
            n_fallen += 1
        if s.major_alert:
            n_alert += 1
    ...
    while True:
        ...
        if face_extractor.available and show_spo2:
            ...
            for fr in face_results:
                ...
                # Sub-region ROI extraction — O(ROI pixels) not O(frame area)
                rgb = fr.mean_rgb(frame)
                if rgb is not None:
                    face_rppg.push_frame_direct(*rgb)
            ...
            if spo2_frame_counter >= SPO2_UPDATE_INTERVAL:
                ...
                # Only process visible faces — skip stale pool entries
                for fr in face_results:
                    pool_entry = rppg_pool.get(fr.face_id)
